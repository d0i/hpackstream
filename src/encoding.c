...
// section 4.1.1
int enc_integer(char *buf, int buf_len, int64_t value, int prefix_len){
...
// section 4.1.1
int64_t dec_integer(char *data, int data_len, int prefix_len, char **next_read_pp){
  int64_t value;
  long m;
  int max_first = encmax(8-prefix_len);
  int count = 0;
...
// section 4.1.2
int enc_string(char *buf, int buf_len, char *s, int s_len){
  int i;
  char *wp;
  buf[0] = 0;
...
int dec_string(char *data, int data_len, char *buf, int buf_len){
  int64_t slen;
  char *s;
  if (data[0] & 0x80 == 0x80){
